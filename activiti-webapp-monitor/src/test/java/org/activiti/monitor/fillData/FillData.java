...
public class FillData {
	...
	final private int N_INSTANCES = 5;
	...
	private void completeTasks(String taskname, int percentafe) {
		...
		List<Task> taskList1 = taskService.createTaskQuery().taskName(taskname)
				.list();
		...
		for (ProcessDefinition pd : processDefinitionList) {
			...
			for (int i = 0; i < N_INSTANCES; i++) {
				ProcessInstance processInstance = runtimeService
						.startProcessInstanceById(pd.getId(), "bk" + i,
								randomVariables());
	...
	void deplyProcesses() throws IOException {
		...
		for (String processFileName : processFileNames) {
			...
			for (int i = 1; i <= N_PROCESSES; i++) {
				...
				deployment = deployment.addInputStream("process" + +j + "_" + i
						+ ".bpmn20.xml",
